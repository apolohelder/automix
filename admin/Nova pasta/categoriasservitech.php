...
if(isset($_POST['cadastrar']) && !empty($_POST['cadastrar']))	{
	...
	if(strlen($categoria) != 0){
			 $sel_login = mysqli_query($conn,"SELECT * FROM tbcservitech WHERE categoria='$categoria'");
	...
	$query = mysqli_query($conn,"INSERT INTO tbcservitech (categoria) VALUES ('$categoria')")  or die (mysqli_error());
			
	
	echo '<script> alert("cadastro foi efetuado com sucesso!"); window.location="index.php?pg=categoriasservitech&listar=listar&ativo=3&link=9"; </script>';
...
if(isset($_POST['editar']) && !empty($_POST['editar']))	{
	...
	if(strlen($categoria) != 0){
			 $sel_login = mysqli_query($conn,"SELECT * FROM tbcservitech WHERE categoria='$categoria' && id !='$id'");
			 $conta_login = mysqli_num_rows($sel_login);
		if($conta_login != 0){
			alert('Já existe um cadastro com este nome de categoria.');
	...
	$query = mysqli_query($conn,"UPDATE tbcservitech SET
	 categoria   = '{$_POST['categoria']}'
	 WHERE id  = '{$id}'") or die("Error: " . mysqli_error());
	
	echo '<script> alert("cadastro foi atualizado com sucesso!");  window.location="index.php?pg=categoriasservitech&listar=listar&ativo=3&link=9"; </script>';
...
<section class="content container-fluid">
	...
	<? } if($_GET['listar'] == "listar") { ?>
		<!--<a href="index.php?pg=categoriasservitech&listar=mostrar&ativo=3&link=9"><div class="btn btn-success" style="margin: 10px;">CADASTRAR CATEGORIA</div></a>	-->
    ...
    <tbody>
       <? $query = mysqli_query($conn,"SELECT * FROM tbcservitech");
					while ($dados = mysqli_fetch_array($query)) {
		?>
        <tr id="item_<?=$dados['id'];?>">
          <td><?=$dados['categoria'];?></td>
            <td><a class="btn btn-info" href="index.php?pg=categoriasservitech&listar=editar&id=<?=$dados['id'];?>&ativo=3&link=9">
     ...
     <? }?>
		...
		$query_editar = mysqli_query($conn,"SELECT * FROM tbcservitech WHERE id = {$_GET['id']}");
...
<script>
	...
	$('#confirm-delete').on('show.bs.modal', function(e) {
			...
			$(document).on('click','#botaodeletar',function(e){
				...
				$.ajax({
    url:'excluir.php?acao=deletarcategoriaserv',
    type: 'POST',
    data:'id='+DbNumberID,
