...
if(isset($_POST['cadastrar']) && !empty($_POST['cadastrar']))	{
    ...
    $categoria = $_POST['categoria'];
  	$foto = $_POST['nomefoto'];		
	$categoriaservicos = $_POST['categoriaservicos'];
	
	$query = mysqli_query($conn,"INSERT INTO tbservicos (categoria,foto,categoriaservicos) VALUES ('$categoria','$foto','$categoriaservicos')")  or die (mysqli_error($conn));
	
	echo '<script> alert("Serviço foi cadastrado com sucesso!"); window.location="index.php?pg=servicos&listar=listar&id=1&ativo=2&link=7"; </script>';
...
if(isset($_POST['editar']) && !empty($_POST['editar']))	{
	...
	echo '<script> alert("Serviço foi atualizado com sucesso!");  window.location="index.php?pg=servicos&listar=listar&id=1&ativo=2&link=7"; </script>';
...
<section class="content container-fluid">
		...
		<? if($_GET['listar'] == "mostrar") { ?>
				...
				<form name="cadastro" id="cadastro" method="post" action="">
					...
					<label for="senha2">Categoria</label>
							<div>
					<select name="categoriaservicos" required="required" class="form-control selectpicker" id="categoriaservicos" data-live-search="true">
						<option value="">Escolha uma categoria</option>
                      <? $query = mysqli_query($conn,"SELECT * FROM tbcategoriaservicos order by categoria asc");
				  ...
				  <div class="form-group">
					...
					<select name="categoria" required="required" class="form-control selectpicker" id="categoria" data-live-search="true">
						<option value="">Escolha uma categoria</option>
                      <? $query = mysqli_query($conn,"SELECT * FROM tbcategoria2 order by categoria asc");
						 while($dados = mysqli_fetch_array($query)){ ?>
                      <option value="<?=$dados['id'];?>"><?=$dados['categoria'];?></option>
                     <? }?>
                    </select></div>
  ...
  <span class="form-control"></span>
	...
	<? } if($_GET['listar'] == "listar") { ?>
		<a href="index.php?pg=servicos&listar=mostrar&ativo=2&link=7"><div class="btn btn-success" style="margin: 10px;">CADASTRAR SERVIÇOS</div></a>	
    ...
    <tbody>
       <? $query = mysqli_query($conn,"Select tbservicos.id, tbservicos.foto, tbcategoria2.categoria, tbcategoriaservicos.categoria as categoriaprincipal
from tbservicos
Join tbcategoria2
On tbservicos.categoria = tbcategoria2.id
JOIN tbcategoriaservicos
on tbservicos.categoriaservicos = tbcategoriaservicos.id
");
					while ($dados = mysqli_fetch_array($query)) {
		?>
        <tr id="item_<?=$dados['id'];?>">
		 ...
		 <a class="btn btn-success" href="index.php?pg=galeria3&listar=mostrar&id=<?=$dados['id'];?>&ativo=2&link=7">
              <i class="glyphicon glyphicon-arrow-up icon-white"></i>
              Adicionar Fotos
              </a>
				
		  <a class="btn btn-info" href="index.php?pg=servicos&listar=editar&id=<?=$dados['id'];?>&ativo=2&link=7">
     ...
     <? }?>
		...
		$query_editar = mysqli_query($conn,"Select tbservicos.id, tbservicos.foto, tbcategoria.categoria, tbcategoria.id as id2, tbservicos.categoriaservicos as id3
from tbservicos
Join tbcategoria
On tbservicos.categoria = tbcategoria.id 
JOIN tbcategoriaservicos
ON tbcategoriaservicos.id = tbservicos.categoriaservicos
WHERE tbservicos.id = {$_GET['id']}");
		...
		<form name="editar" id="editar" method="post" action="" enctype="multipart/form-data">
					...
					<? $queryempresa = mysqli_query($conn,"select * from tbcategoriaservicos where id = ".$dados_editar['id3']."");
					   $dadosempresa = mysqli_fetch_array($queryempresa);?>
							<select name="categoriaservicos" class="form-control selectpicker" id="categoriaservicos" data-live-search="true">
                      <option value="<?=$dadosempresa['id'];?>" selected="selected"><?=$dadosempresa['categoria'];?></option>
					<? $queryempresa2 = mysqli_query($conn,"select * from tbcategoriaservicos where id != ".$dados_editar['id3']."");
					...
					<? $queryempresa = mysqli_query($conn,"select * from tbcategoria2 where id = ".$dados_editar['id2']."");
					   $dadosempresa = mysqli_fetch_array($queryempresa);?>
							<select name="categoria" class="form-control selectpicker" id="categoria" data-live-search="true">
                      <option value="<?=$dadosempresa['id'];?>" selected="selected"><?=$dadosempresa['categoria'];?></option>
					<? $queryempresa2 = mysqli_query($conn,"select * from tbcategoria2 where id != ".$dados_editar['id2']."");
					   while($dadosempresa2 = mysqli_fetch_array($queryempresa2)) { ?>
  ...
  <span class="form-control"></span>
			...
			<div class='preview'>
                ...
                <img src="uploadprodutos2/default2.png" id="img" width="200">
				<? }else{ ?>
			  <img src="uploadprodutos2/thumbs/<?=$dados_editar['foto'];?>" id="img" width="200">
				<? } ?>
...
<script>
	...
	$('#confirm-delete').on('show.bs.modal', function(e) {
			...
			$(document).on('click','#botaodeletar',function(e){
				...
				$.ajax({
    url:'excluir.php?acao=deletarservico',
    type: 'POST',
    data:'id='+DbNumberID,
