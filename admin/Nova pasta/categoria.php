...
if(isset($_POST['cadastrar']) && !empty($_POST['cadastrar']))	{
	...
	if(strlen($categoria) != 0){
			 $sel_login = mysqli_query($conn,"SELECT * FROM tbcategoria WHERE categoria='$categoria'");
	...
	$query = mysqli_query($conn,"INSERT INTO tbcategoria (categoria,descricao,url) VALUES ('$categoria','$descricao','$url')")  or die (mysqli_error());
			
	
	echo '<script> alert("cadastro foi efetuado com sucesso!"); window.location="index.php?pg=categoria&listar=mostrar&ativo=1&link=20"; </script>';
...
if(isset($_POST['editar']) && !empty($_POST['editar']))	{
	...
	if(strlen($categoria) != 0){
			 $sel_login = mysqli_query($conn,"SELECT * FROM tbcategoria WHERE categoria='$categoria' && id !='$id'");
			 $conta_login = mysqli_num_rows($sel_login);
		if($conta_login != 0){
			alert('Já existe um cadastro com este nome de categoria.');
	...
	echo '<script> alert("cadastro foi atualizado com sucesso!");  window.location="index.php?pg=categoria&listar=listar&ativo=1&link=20"; </script>';
...
<section class="content-header">
      <h1>
        Categoria de produtos<small></small>
      </h1>
      <ol class="breadcrumb">
...
<section class="content container-fluid">
	...
	<? } if($_GET['listar'] == "listar") { ?>
		<a href="index.php?pg=categoria&listar=mostrar&link=4&ativo=1"><div class="btn btn-success" style="margin: 10px;">CADASTRAR CATEGORIA</div></a>	
    ...
    <tbody>
       <? $query = mysqli_query($conn,"SELECT * FROM tbcategoria");
					while ($dados = mysqli_fetch_array($query)) {
		?>
        <tr id="item_<?=$dados['id'];?>">
          <td align="center"><input type="checkbox" id="<?=$dados['id'];?>"  name="favorite" value="<?=$dados['status'];?>" class="js-switch" <?=$dados['status'] == '1' ? 'checked' : '' ;?>/> </td>
            <td><b><?=$dados['categoria'];?></b> - <?=$dados['descricao'];?></td>
            <td><a class="btn btn-info" href="index.php?pg=categoria&listar=editar&id=<?=$dados['id'];?>&ativo=1&link=4">
     ...
     <? }?>
		...
		$query_editar = mysqli_query($conn,"SELECT * FROM tbcategoria WHERE id = {$_GET['id']}");
...
<script>
	...
	$('#confirm-delete').on('show.bs.modal', function(e) {
			...
			$(document).on('click','#botaodeletar',function(e){
				...
				$.ajax({
    url:'excluir.php?acao=deletarcategoria',
    type: 'POST',
    data:'id='+DbNumberID,
