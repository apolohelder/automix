...
if(isset($_POST['cadastrar']) && !empty($_POST['cadastrar']))	{	
	...
	$query = mysqli_query($conn,"INSERT INTO tbbannerservitech (foto) VALUES ('$foto')")  or die (mysqli_error($conn));
	
	echo '<script> alert("Banner foi cadastrado com sucesso!"); window.location="index.php?pg=banner_servitech&listar=editar&id=1&ativo=3&link=6"; </script>';
...
if(isset($_POST['editar']) && !empty($_POST['editar']))	{	
	...
	$query = mysqli_query($conn,"UPDATE tbbannerservitech SET
	
	foto = '{$_POST['nomefoto']}'

	 WHERE id  = '{$id}'") or die("Error: " . mysqli_error($conn));
	
	echo '<script> alert("Imagem foi atualizada com sucesso!");  window.location="index.php?pg=banner_servitech&listar=editar&id=1&ativo=3&link=6"; </script>';
...
<section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Lista de imagem servitech</li>
      </ol>
    </section>
...
<section class="content container-fluid">
				...
				<form name="cadastro" id="cadastro" method="post" action="">
					<div class="col-md-6">
						...
						<div class="form-group">
							<label for="foto">Imagem servitech*(largura mínima: 1920px)</label>
  ...
  <span class="form-control"></span>
	...
	<? } if($_GET['listar'] == "listar") { ?>
		<a href="index.php?pg=banner_servitech&listar=mostrar&ativo=16&link=25"><div class="btn btn-success" style="margin: 10px;">CADASTRAR BANNER</div></a>	
    ...
    <tbody>
       <? $query = mysqli_query($conn,"Select * from tbbannerservitech");
				while ($dados = mysqli_fetch_array($query)) {
		?>
        <tr id="item_<?=$dados['id'];?>">
            <td><img src="uploadbanner/thumbs/<?=$dados['foto'];?>"/></td>
            <td>
			
					
		  <a class="btn btn-info" href="index.php?pg=banner_servitech&listar=editar&id=<?=$dados['id'];?>&ativo=16&link=25">
...
from tbbannerservitech
WHERE tbbannerservitech.id = {$_GET['id']}");
  ...
  <span class="form-control"></span>
			...
			<div class='preview'>
                ...
                <img src="uploadbanner/default2.png" id="img" width="200">
				<? }else{ ?>
			  <img src="uploadbanner/thumbs/<?=$dados_editar['foto'];?>" id="img">
				<? } ?>
...
<script>
	...
	$('#confirm-delete').on('show.bs.modal', function(e) {
			...
			$(document).on('click','#botaodeletar',function(e){
				...
				$.ajax({
    url:'excluir.php?acao=deletarbanner',
    type: 'POST',
    data:'id='+DbNumberID,
