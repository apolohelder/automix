...
include('class.upload.php');
	...
	$nome_da_imagem = $handle->file_new_name_body = $senha;		
        ...
        $handle->Process('uploadfotos2/original');
        
        $handle->image_resize            = true;       
		$handle->image_ratio_x           = true;       	
		$handle->image_y                 = 200;
		$handle->file_new_name_body = "$nome_da_imagem";       
        $handle->Process('uploadfotos2/thumbs/');
        
		$handle->image_resize            = true;       
		$handle->image_ratio_x           = true;       	
		$handle->image_y                 = 500;
		$handle->file_new_name_body = "$nome_da_imagem";       
        $handle->Process('uploadfotos2/media/');
...
#echo $nomeimagem;
	...
	$query = mysqli_query($conn,"INSERT INTO tbgaleriafotos2 (idgaleria,foto) VALUES ('$idgaleria','$nomeimagem')")  or die (mysqli_error($conn));
