...
#echo $nomeimagem;
    ...
    $idgaleria = intval($_POST['galleryid']);
	
	$contagem_destaque = mysqli_query($conn,"SELECT idgaleria,destaque FROM tbgaleriafotos WHERE idgaleria = ".$idgaleria."");
	$total = mysqli_num_rows($contagem_destaque);
	if($total==0) {
		$destaque = 1;
	}else{
		$destaque = 0;
	...
	$query = mysqli_query($conn,"INSERT INTO tbgaleriafotos (idgaleria,foto,destaque) VALUES ('$idgaleria','$nomeimagem','$destaque')")  or die (mysqli_error($conn));
