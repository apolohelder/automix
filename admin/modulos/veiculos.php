...
$destaque = 0;
		...
		if(strlen($placa) != 0){
			 $sel_login = mysqli_query($conn,"SELECT * FROM ".$tabela." WHERE placa='$placa'");
			 $conta_login = mysqli_num_rows($sel_login);
		if($conta_login != 0){
		
				echo '<script> bootbox.alert("Já existe um cadastro de veículo com esta placa.")</script>';
		}else{	
		
	
	$query = mysqli_query($conn,"INSERT INTO ".$tabela." (marca,modelo,versao,km,cor,anofabricacao,anomodelo,combustivel,portas,cambio,estado,municipio,ofertas,placa,valor,descricao,opcionais,informacoes,status,nvisitas,destaque) VALUES ('$marca','$modelo','$versao','$km','$cor','$anofabricacao','$anomodelo','$combustivel','$portas','$cambio','$estado','$municipio','$ofertas','$placa','$valor','$descricao','$opcionais','$informacoes','$status','$nvisitas','$destaque')")  or die (mysqli_error($conn));
	$last_id = mysqli_insert_id($conn);
	//print $last_id;
			
	echo '<script> bootbox.alert("Cadastro foi efetuado com sucesso!<br>Na próxima tela, adicione as fotos.", function(){  location.href = "'.URL::getBase().''.$pagina.'/listar/editarfotos/id/'.$last_id.'";  })</script>';
...
$(document).ready(function(){
  // $('input[name=destaque]').on("click",function(){
	$('input[name=destaque2]').on('ifChecked ifUnchecked', function(event){   
	var id    = $(this).prop('id');
	...
	$.ajax({
		type:'POST',
		url:'<?=URL::getBase(); ?>modulos/updatedestaque.php',
		data:'id= ' + id + '&destaque='+destaque
	});
	console.log('id: ' + id );
  ...
  <div class="col-md-4"><div class="form-group">
		  ...
		  <div class="col-md-4 d-none">
			<div class="form-group">
						  ...
						  <input name="valor" type="text" class="form-control" id="valor" placeholder="Ex: 27.500">
			</div>
		  </div>
</div>
...
<div class="row">
  ...
  <div class="col-md-4 d-none"><div class="form-group">
						  <label for="endereco">Valor</label>
						  <input name="valor" type="text" class="form-control" id="valor" placeholder="Ex: 27.500" value="<?=$dados_editar['valor'];?>">
						</div></div>
</div>
	  ...
	  <div class="row">
		<div class="col-md-6 col-sm-12">
		  ...
		  <div id="drag-and-drop-zone" class="dm-uploader p-5">
			...
			<div class="btn btn-primary btn-block mb-5 btn-photo">
...
<div class="mt-2">
	...
	<button type="button" class="btn btn-info" onClick="history.go(-1)">Voltar</button>
	  ...
	  <div class="row">
		<div class="col-12"></div>
			...
			<div class="callout alert-primary" style="border-left: 5px solid #004085;">
				<p>Lista de fotos cadastradas nesta galeria.</p>
			</div>
			<div class="callout alert-success">
				<p>Selecione uma foto para ser destaque do anúncio.</p>
			</div>
   ...
   ?>
	...
	{
   ?>
	 <div class="col-md-2"><div class="row" id="item_<?=$dados["id"];?>"><div class="img-thumbnail"><img class="img-car img-fluid" src="<?=URL::getBase(); ?>uploadfotos/thumbs/<?=$dados["foto"]; ?>" id='img_<?=$dados["id"];?>' width="220">

	 <div id='delete_<?=$dados["id"];?>' class="btn btn-block btn-success"><span class="mr-2">Destaque</span><input type="radio" id="<?=$dados['id'];?>"  name="destaque2" value="<?=$dados['destaque'];?>" <?=$dados['destaque'] == '1' ? 'checked' : '' ;?>/></div>  
	 <div id='delete_<?=$dados["id"];?>' class="btn btn-block btn-danger"><span class="mr-2">Excluir</span> <input type="checkbox" name="customer_id[]" class="delete_customer" value="<?=$dados["id"]; ?>" /></div>
	  ...
	  <? } ?> 
			...
			<? }  ?>
			  <div align="center">
	<button type="button" name="btn_delete" id="btn_delete" class="btn btn-danger mt-3">Deletar arquivos selecionados</button>
   </div><? }  ?>
		</div> </div>
		<!--/.col (left) -->
		<!-- right column -->
