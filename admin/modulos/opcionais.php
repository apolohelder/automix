...
if(isset($_POST['cadastrar']) && !empty($_POST['cadastrar']))	{
	...
	if(strlen($opcional) != 0){
			 $sel_login = mysqli_query($conn,"SELECT * FROM ".$tabela." WHERE opcional='$opcional'");
			 $conta_login = mysqli_num_rows($sel_login);
		if($conta_login != 0){
			echo '<script> bootbox.alert("Já existe um cadastro com este nome de opcional.")</script>';
		}else{	
	
	
	$query = mysqli_query($conn,"INSERT INTO ".$tabela." (opcional) VALUES ('$opcional')")  or die (mysqli_error());
...
if(isset($_POST['editar']) && !empty($_POST['editar']))	{
	...
	if(strlen($opcional) != 0){
			 $sel_login = mysqli_query($conn,"SELECT * FROM ".$tabela." WHERE opcional='$opcional' && id !='$id'");
			 $conta_login = mysqli_num_rows($sel_login);
		if($conta_login != 0){
			echo '<script> bootbox.alert("Já existe um cadastro com este nome de opcional.")</script>';
...
<section class="content-header">
      <h1>
        Gerenciamento de opcionais de série do veículo<small></small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Menu</a></li>
        <li class="active">Lista de opcionais de série do veículo</li>
      </ol>
    </section>
...
<section class="content container-fluid">
				...
				<form name="cadastro" id="cadastro" method="post" action="">
					<div class="col-md-6">
						...
						<div class="form-group">
						  ...
						  <input name="opcional" type="text" required class="form-control" id="opcional" placeholder="Ex: Ar-Condicionado">
						</div>							
	...
	<? }    if(Url::getURL(2) == "listar") { ?>
		<a href="<?=URL::getBase(); ?><?=$pagina;?>/listar/mostrar"><div class="btn btn-success" style="margin: 10px;">CADASTRAR OPCIONAL</div></a>	
		<table id="example" class="table table-hover table-striped table-bordered" width="100%" cellspacing="0">
    <thead class="table-dark">
        <tr>
          <th width="80%">OPCIONAIS</th>
    ...
    <tbody>
       <? $query = mysqli_query($conn,"SELECT * FROM ".$tabela."");
        ...
        <tr id="item_<?=$dados['id'];?>">
          <td><a href="<?=URL::getBase(); ?><?=$pagina;?>/listar/editar/id/<?=$dados['id'];?>"><?=$dados['opcional'];?></a></td>
     ...
     <? }?>
		...
		<form method="post" name="editar" id="editar" action="">
					...
					<div class="col-md-6">
						<div class="form-group">
							<label for="opcional">Opcional</label>
							<input name="opcional" type="text" required class="form-control" id="opcional" placeholder="Ex: Ar-Condicionado" value="<?=$dados_editar['opcional'];?>">
						</div>					
