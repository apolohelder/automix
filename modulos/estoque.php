...
if(!empty($_GET['marca']) || (!empty($_GET['cambio'])) || (!empty($_GET['combustivel'])) || (!empty($_GET['portas'])) || (!empty($_GET['anofabricacao'])) ) {
$marca = $_GET['marca'];
$cambio = $_GET['cambio'];
...
on mo.id = ve.modelo";
		...
		if (!empty($cambio))	{  			
			$query_buscar .= $cond . " cambio LIKE '" .$cambio."'";
			$cond = " AND ";			
		}
		if (!empty($combustivel))	{  			
			$query_buscar .= $cond . " combustivel LIKE '" .$combustivel."'";
			$cond = " AND ";			
		}
		if (!empty($portas))	{  			
			$query_buscar .= $cond . " portas LIKE '" .$portas."'";
			$cond = " AND ";			
		}
		if (!empty($anofabricacao))	{  			
			$query_buscar .= $cond . " anofabricacao LIKE '" .$anofabricacao."'";
			$cond = " AND ";
...
$query_buscar = "SELECT ve.id,ve.cambio,ve.combustivel,ve.portas,ve.anofabricacao,ve.versao,ve.anomodelo,ve.km,ve.valor,ve.marca, mo.modelo as modelove, ma.marca as marcave, ve.ofertas as ofertas2,  (SELECT foto FROM tbgaleriafotos WHERE idgaleria = ve.id order by id ASC LIMIT 1) AS foto FROM tbveiculos as ve
Join tbmarcas as ma
On ma.id = ve.marca
JOIN tbmodelos as mo 
on mo.id = ve.modelo";
		...
		<div class="col-md-8">
			<div class="row">
				...
				<div class="col-md-4 col-lg-4 col-xs-12 mb-4">
					...
					<img class="img-fluid ng-lazyloaded img-car2" src="<?=URL::getBase() ?>admin/uploadfotos/thumbs/<?=$dados['foto'];?>"></a>
					<div class="card-body">
						...
						<div class="pb-3 clearfix border-bottom">
							<a href="<?=URL::getBase() ?>veiculo/<?=$dados['id'];?>/<?=url_amigavel($dados['versao'].'-'.$dados['marcave'].'-'.$dados['modelove']);?>" class="float-left bg-automix p-2 leads b-9 text-white px-3" style="font-size: 15px;"> Veja mais </a>
							<!-- span class="float-left bg-automix p-2 leads b-9 text-white" style="font-size: 15px;"> R$ <?=number_format($dados['valor'], 2, ',', '.');?> </span -->
							<span class="float-right p-2 f-09 text-muted" style="font-size: 14px;">
				...
				<ul class="pagination">
					<li class="page-item">
						...
						if($pagina_anterior != 0){ ?>
							<a class="page-link" href="<? echo URL::getBase() ?>estoque/pagina/<?=$pagina_anterior; ?>/?/<?=$params;?>" aria-label="Previous">
					...
					<li class="page-item"><a class="page-link" href="<? echo URL::getBase() ?>estoque/pagina/<?=$i;?>/?/<?=$params;?>"><? echo $i; ?></a></li>
					
					<? } } ?>
					<li class="page-item">
						<?
						if($pagina_posterior <= $num_pagina){ ?>
							<a class="page-link" href="<? echo URL::getBase() ?>estoque/pagina/<?=$pagina_posterior;?>/?/<?=$params;?>" aria-label="Previous">
